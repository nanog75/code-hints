...
if __name__ == '__main__':
    ...
    if nc_mgr is None:
        ...
        for filename in downloaded_xml_filepaths:
             with open(filename, 'r') as yang_xml:
                 response= nc_mgr.edit_config(yang_xml.read(), format='xml', target='candidate',
                                         default_operation='merge')
                 ztp_script.syslogger.info(response)
                 print(response)
                 response_dict=xmltodict.parse(str(response))
                 print json.dumps(response_dict, indent=4)

                 response=nc_mgr.commit()
                 ztp_script.syslogger.info(response)
                 print(response)
                 response_dict=xmltodict.parse(str(response))
                 print json.dumps(response_dict, indent=4)
